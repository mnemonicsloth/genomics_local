...
graphDir='/home/jd/src/file-of-genomes/gcskew-graphs/'
...
def plotGCSkewForRefGenome(dir, species_dict):
    file_to_plot = get_reference_genome(dir)
    chunk_size = 10000
    bufs = read_corresponding_fna(file_to_plot)
    revised_bufs = [(head, text) for head, text in bufs
                    if head in species_dict[file_to_plot].keys()]
    if revised_bufs == []:
        raise Exception(filename)
    for chromosomeID, text in revised_bufs:
        tally = [do_chunk(c) for c in partition_chunks(text, chunk_size)]
        ...
        plt.title(f"{dir}:{chromosomeID}")
        filename = f"/home/jd/src/file-of-genomes/gcskew-graphs/{dir}-{chromosomeID}.pdf"
...
def doDirectory(file_obj, dir):
    print("doDirectory printed_dir=" + dir)
    d = process_families_file(dir)
    plotGCSkewForRefGenome(dir, d)
    for file, genome in d.items():
        oriterm = getOriTerm(file, d)
        for chromosomeID, (ori, term, n) in oriterm.items():
            file_obj.write(f"{dir} {file} {chromosomeID} {ori} {term} {n}\n")

            
def buildFileOriTermDirectory():
    os.chdir("/home/jd/src/file-of-genomes/ccout")
    file_obj = open("/home/jd/src/file-of-genomes/directory.txt", "w")
    for directory in os.listdir():
        print("buildFileOriTermDirectory() dir=" + directory)
        doDirectory(file_obj, directory)
    file_obj.close()
